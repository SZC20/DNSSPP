...
class  LaplaceApproximation( ModelAdapter) :
    ...
    def optimize(self, optimizer = opt_type.AUTO_DIFF, maxiter = None, tol = None, verbose=True, nos = False, **kwargs):
        ...
        if  optimizer is opt_type.AUTO_DIFF :
            ...
            def objective_closure():
                if nos:
                    self.lrgp.fit_nos(sample = False)
                else:
                    self.lrgp.fit(sample = False)
                minus_lml = -1 * self.log_marginal_likelihood()
                # print(minus_lml)
                return minus_lml
    ...
    def optimize_deep(self, optimizer = opt_type.AUTO_DIFF, maxiter = None, tol = None, verbose=True, nos = False, epoch = 100, wage_decay = False, learning_rate = 0.001, **kwargs):
        ...
        for i in range(epoch):
            ...
            with tf.GradientTape() as tape:
                loss = -1 * self.log_marginal_likelihood() 
            grads = tape.gradient(loss, self.lrgp.network.trainable_variables)
            optimizer.apply_gradients(zip(grads, self.lrgp.network.trainable_variables))
            
        print('optimize_deep finished.')
        print(self.log_marginal_likelihood())
        # print(self.lrgp.network.trainable_variables)
        ...
        # if  optimizer is opt_type.AUTO_DIFF :
            ...
            # trainable_variables_tuple = tuple(self.lrgp.network.trainable_variables)

            # res = OptimScipyAutoDiff().minimize(objective_closure, trainable_variables_tuple, method = 'L-BFGS-B', options = kwargs )
